...
void IApplicationBase::_events()
{
	...
	while( SDL_PollEvent(&event) )
	{
		...
		{
			SDL_MouseWheelEvent * mouse_wheel_event = (SDL_MouseWheelEvent*)&event;

			if( event.wheel.y > 0 )
			{
				if( m_Window->get_window_type() == WT_OPENGL )
					mouse_wheel_up(mouse_wheel_event->x, CConfig::WindowHeight-mouse_wheel_event->y);
				else
					mouse_wheel_up(mouse_wheel_event->x, mouse_wheel_event->y);
			}
			else if( event.wheel.y < 0 )
			{
				if( m_Window->get_window_type() == WT_OPENGL )
					mouse_wheel_down(mouse_wheel_event->x, CConfig::WindowHeight-mouse_wheel_event->y);
				else
					mouse_wheel_down(mouse_wheel_event->x, mouse_wheel_event->y);
			}
		}*/
...
void IApplicationBase::window_resized()
{
	render();
}

void IApplicationBase::window_exposed()
{
	render();
}
