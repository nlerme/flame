...
CSDLWindow::CSDLWindow( const std::string & title, const int width, const int height, const int flags, const bool centered ) : IWindow(title, width, height, flags, centered)
{
...
void CSDLWindow::swap_buffers() const
{
	SDL_Flip(m_Screen);
}

void CSDLWindow::resize( const int width, const int height ){}

void CSDLWindow::clear( const color4f & c )
{
	SDL_FillRect(m_Screen, NULL, SDL_MapRGB(m_Screen->format, Uint8(c.r*255.0f), Uint8(c.g*255.0f), Uint8(c.b*255.0f)));
}

void CSDLWindow::set_pixel( const int x, const int y, const color4f & c )
{
...
color4f CSDLWindow::get_pixel( const int x, const int y ) const
{
...
void CSDLWindow::draw_rectangle( const int x1, const int y1, const int x2, const int y2, const color4f & c )
{
	int x(std::min(x1, x2));
	int y(std::min(y1, y2));
	int width(abs(x1-x2));
...
void CSDLWindow::fill_circle( const int xc, const int yc, const int radius, const color4f & c ){}
...
EWindowType CSDLWindow::get_window_type() const
{
	return WT_SDL;
}
